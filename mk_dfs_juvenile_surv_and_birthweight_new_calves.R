...
year_juve=year_plus_est_death%>%
  mutate(juvenile_survival = case_when(
    ## when no death year but calf last seen during first yr of life - prbly died 
...
table(juvenile_surv$juvenile_survival) # slightly inflated as 2023 ids have not been through second winter
# will remove 2023 ids at later stage
...
## FROH values ##
##############################################
FROH_full<-read.table("Deer_SNP_2024/ROH_search_2024.hom.indiv", header=T, stringsAsFactors = F)%>%
  select(IID,KB) %>% rename(Code=IID)%>%mutate(FROH=KB/2591865)%>%
  filter(nchar(Code)==5)%>% #removing IDs with non-sensical ID codes
  select(-KB)

FROH_mum<-read.table("Deer_SNP_2024/ROH_search_2024.hom.indiv", header=T, stringsAsFactors = F)%>%
...
surv_loc_df= juvenile_loc_surv_bw%>%
  filter(BirthYear<=2022)%>% #removing 2023 cohort as havent gone through second winter yet.
  select(-mum_birthyear, -CaptureWt, -AgeHrs) %>%
  drop_na(FROH, juvenile_survival, N, E)
...
bw_loc_df=juvenile_loc_surv_bw%>%
  ...
  drop_na(FROH,CaptureWt, N , E)

head(bw_loc_df)
nrow(bw_loc_df)

all=juvenile_loc_surv_bw%>%
  drop_na(FROH, N , E)



write.table(surv_loc_df,
            file = "Deer_spatial_variation_ID/survival_loc_2024_new_calves.txt",
            row.names = F, quote = F, sep = ",",na = "NA") #saving tables as txt file 


write.table(bw_loc_df,
            file = "Deer_spatial_variation_ID/birthweight_loc_2024_new_calves.txt",
            row.names = F, quote = F, sep = ",",na = "NA") #saving tables as txt file 


write.table(all,
            file = "Deer_spatial_variation_ID/df_loc_2024_new_calves_all.txt",
            row.names = F, quote = F, sep = ",",na = "NA") #saving tables as txt file 
